...
class _ICP():
    ...
    def train(self, x_all, y_all, opt_params, is_init, indices_x,
        ...
        for epoch in range(opt_params.epochs):#100
            if self.verbose:
                print("_" * 98)
    ...
    def loss_cal(self,TX, TY, x_all, y_all):
        d, n = x_all.shape
        x_est = TY.dot(y_all).astype(np.float32) #y_all: [25,5000] TY:(25,25)
        y_est = TX.dot(x_all).astype(np.float32)
        res = faiss.StandardGpuResources() 
        nbrs_x = faiss.IndexFlatL2(d) #d=25
        nbrs_x = faiss.index_cpu_to_gpu(res, 0, nbrs_x)
        nbrs_x.add(np.ascontiguousarray(x_est.T))
        _, indices_x = nbrs_x.search(x_all.T, k=1) #indices_x: (5000,1)
        indices_x = indices_x.squeeze() # nearest nbrs of true x in x estimated dataset (transformed true y)

        nbrs_y = faiss.IndexFlatL2(d)
        nbrs_y = faiss.index_cpu_to_gpu(res, 0, nbrs_y)
        nbrs_y.add(np.ascontiguousarray(y_est.T))
        _, indices_y = nbrs_y.search(y_all.T, k=1)#y_all.T:(5000,25)
        indices_y = indices_y.squeeze() # nearest nbrs of true y in y estimated dataset (transformed true x)
        rec_x = ((x_all - x_est[:, indices_x]) ** 2).sum(0).mean() # result:number
        rec_y = ((y_all - y_est[:, indices_y]) ** 2).sum(0).mean()

        bb_idx_y = np.where(indices_x[indices_y] == np.arange(n))[0] # length is 506
        bb_idx_x = np.where(indices_y[indices_x] == np.arange(n))[0]
        return rec_x, rec_y, min(len(bb_idx_x), len(bb_idx_y))
    
    def train_epoch(self, x_all, y_all, is_init, indices_x,
                    indices_y, epoch, do_reciprocal=False):
        # Compute batch size
        batch_size = self.opt_params.batch_size #128
        d, n1, n2 = x_all.shape[0], x_all.shape[1], y_all.shape[1]
        rpa = np.random.permutation(n1) # random shuffle from 1 to 5000
        rpb = np.random.permutation(n2) # random shuffle from 1 to 5000
       
        x_est = self.TY.dot(y_all).astype(np.float32) #y_all: [25,5000] TY:(25,25)
        ...
        if not is_init or epoch > 0:
            ...
            nbrs_x = faiss.index_cpu_to_gpu(res, 0, nbrs_x)
            nbrs_x.add(np.ascontiguousarray(x_est.T))
            _, indices_x = nbrs_x.search(np.ascontiguousarray(x_all.T), k=1) #indices_x: (5000,1)
            indices_x = indices_x.squeeze() # nearest nbrs of true x in x estimated dataset (transformed true y)

            nbrs_y = faiss.IndexFlatL2(d)
            nbrs_y = faiss.index_cpu_to_gpu(res, 0, nbrs_y)
            nbrs_y.add(np.ascontiguousarray(y_est.T))
            _, indices_y = nbrs_y.search(np.ascontiguousarray(y_all.T), k=1)#y_all.T:(5000,25)
            indices_y = indices_y.squeeze() # nearest nbrs of true y in y estimated dataset (transformed true x)
        ...
        rec_y = ((y_all - y_est[:, indices_y]) ** 2).sum(0).mean()
        # print(time.time()-t)
        bb_idx_y = np.where(indices_x[indices_y] == np.arange(len(indices_y)))[0] # length is 506
        bb_idx_x = np.where(indices_y[indices_x] == np.arange(len(indices_x)))[0]
        
        # print(rec_x, rec_y, bb_idx_x, bb_idx_y)
        # print(self.loss_cal(self.TX, self.TY, x_all, y_all))
        if self.verbose:
        ...
        if do_reciprocal:
            ...
            rpa = np.random.permutation(n1)
            rpb = np.random.permutation(n2)
            batch_n = min(n1, n2) // batch_size #first time: no reciprocal, here

        x_t_all = x_all[:, indices_y] #x_t_all, x_all: both(25,5000)
        ...
        cyc_k = self.opt_params.cyc_k #0.01   what is cyc
        weight_decay = 1e-5
       ...
       # optX = SGD(lr, 0.0, weight_decay)
        ...
        for i in range(batch_n):
            ...
            idx_np = i * batch_size + np.arange(batch_size) # all index in a batch

            idx = rpa[idx_np] # [128,]
            if do_reciprocal: # only use reciprocal points
                idx = bb_idx_x[idx]

            x = x_all[:, idx] # (25,128)
            y_t = y_t_all[:, idx] # (25,128)
            er_x, grad_x_TX = mse_fn(x, y_t, self.TX) #number, [25,25]
