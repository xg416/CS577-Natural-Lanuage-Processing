...
class BiLSTM(nn.Module):
    ...
    def _get_lstm_feature(self, sentence, inputs_pref, len_sen):
        ...
        embeds = nn.Sigmoid()(self.embed(sentence))
        embeds = embeds.view(len_sen, 1, -1)
...
def model1(train_set, test_set, epoch = 20):
    ...
    optimizer = optim.Adam(RIE.parameters(), lr=0.001, betas=(0.9, 0.999), weight_decay=0.002)
...
def model2(train_set, test_set, w2v_path, epoch):
    ...
    W2V = W2VEmbedding(embedding_dim, tag_embedding_dim, 
                            size_prefix = 6, size_tag = 4, context_size = (window_size*2+1))
    optimizer = optim.Adam(W2V.parameters(), lr=0.001, betas=(0.9, 0.999), weight_decay=0.002)
...
def model3(train_set, test_set, w2v_path, epoch):
    ...
    biLSTM = BiLSTM(embedding_dim, tag_embedding_dim, hidden_dim, lstm_layer = 2,
                            size_prefix = 6, size_tag = 4, context_size = (window_size*2+1))
    optimizer = optim.Adam(biLSTM.parameters(), lr=0.001, betas=(0.9, 0.999), weight_decay=0.001)
...
def main():
    ...
    w2v_path = "/homes/cs577/hw2/w2v.bin"

    if args.option == 1:
        model1(train_set, test_set, epoch = 20)
    elif args.option == 2:
        model2(train_set, test_set, w2v_path, epoch = 20)
    elif args.option == 3:
        model3(train_set, test_set, w2v_path, epoch = 20)
    else:
