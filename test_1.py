...
wv_from_bin = KeyedVectors.load_word2vec_format(datapath(r"D:\S\CS577\HW\HW2\w2v.bin"), binary=True)
'''
...
data_dic, vocab_size, train_data =  option1_trainset(data4train, window_size)
train_data = train_regrouping(train_data)
valid_data = option1_vt(data_dic, 'OUT_VOC', data4validate, window_size)
test_data = option1_vt(data_dic, 'OUT_VOC', test_set, window_size)
...
RIE = RandInitEmbedding(vocab_size, embedding_dim, tag_embedding_dim, 
                        size_prefix = 6, size_tag = 4, context_size = (window_size*2+1))
optimizer = optim.Adam(RIE.parameters(), lr=0.001, betas=(0.9, 0.999), weight_decay=0.0005)
...
for epoch in range(total_epoch):
    ...
    train_sample = 0
    train_losses = []
    for data_idx, prefix, pre_tag, label in batch_generate(train_data, batch_size):
        train_sample += 1
        context_idxs = torch.tensor(data_idx, dtype=torch.long)
        ...
        RIE.zero_grad()
        log_probs = RIE(context_idxs, torch_prefix, pre_tag_idx, batch_size)
        loss = loss_function(log_probs, train_label)
        loss.backward()
        optimizer.step()
        
        train_total_loss += loss.item()
    train_ave_loss = train_total_loss / train_sample
    ...
    # when testing, all training data is used to train so no need to validate
        ...
        for v_sen in valid_data:
            ...
            for i in range(len(v_sen)):
                val_num += 1
                data_idx, pre_tag, label, fo_tag, prefix = v_sen[i]
                gt.append(label)
                context_idxs = torch.tensor(data_idx, dtype=torch.long)
                torch_prefix = torch.tensor(prefix, dtype=torch.float)
                pre_tag_idx = torch.tensor(pre_tag, dtype=torch.long)
                ...
                if i == 0:
                    init_probs = RIE(context_idxs, torch_prefix, pre_tag_idx, 1)
                    viterbivars = init_probs.detach().numpy()
                    loss = loss_function(init_probs, valid_label)
                else:
                    gold_probs = RIE(context_idxs, torch_prefix, pre_tag_idx, 1)
                    loss = loss_function(gold_probs, valid_label)
                    transition = RIE.Viterbi_trans(context_idxs, torch_prefix, 1)
        ...
        '''
        print('precision 1, 2, 3:', sklearn.metrics.precision_score(groud_truth, prediction, [0],average='micro'),
              sklearn.metrics.precision_score(groud_truth, prediction, [1],average='micro'),
              sklearn.metrics.precision_score(groud_truth, prediction, [2],average='micro'))
        print('recall 1, 2, 3:', sklearn.metrics.recall_score(groud_truth, prediction, [0],average='micro'),
              sklearn.metrics.recall_score(groud_truth, prediction, [1],average='micro'),
              sklearn.metrics.recall_score(groud_truth, prediction, [2],average='micro'))
        print('f1 score 1, 2, 3:', sklearn.metrics.f1_score(groud_truth, prediction, [0],average='micro'),
              sklearn.metrics.f1_score(groud_truth, prediction, [1],average='micro'),
              sklearn.metrics.f1_score(groud_truth, prediction, [2],average='micro'))
        '''
        print('validation:', epoch, acc, precision, recall, f1)
        print(validation_total_loss / val_num)
